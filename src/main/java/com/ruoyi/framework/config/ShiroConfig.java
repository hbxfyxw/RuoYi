...
public class ShiroConfig
{
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(SecurityManager securityManager)
    {
        ...
        // 不需要拦截的访问
        filterChainDefinitionMap.put("/login", "anon"); //匹配到这个 请求截至到这儿
        ...
        // 注销成功，则跳转到指定页面
        filters.put("logout", logoutFilter());//自定义的会覆盖之前的 LinkedHashMap 属性，同名后面的覆盖前面的
        shiroFilterFactoryBean.setFilters(filters);   //orginchain

        // 所有请求需要认证
        filterChainDefinitionMap.put("/**", "authc");
        // 系统请求记录当前会话
        filterChainDefinitionMap.put("/main", "onlineSession,syncOnlineSession"); //注意顺序
